 ...
 * @return array
 */
function validate_add_project_form($conn, $user_id, $project_name)
{
    $errors = [];
    if (!$project_name) {
...
function validate_add_task_form($conn, $user_id, $task_name, $project_id, $deadline)
{
    $errors = [];
    if (!$task_name) {
...
function validate_user_registration($conn, $email, $password, $login)
{
    $errors = [];
    if (!$email) {
